...
export async function getCompanyCountry(req, res, next) {
    try {
...
export async function getCompanyItems(req, res, next) {
    try {
        if (!req.params || !req.query)
            throw { status: 400, data: "Bad request" };
        if (req.params.type === "flights") {
            const date = req.query.date.toString();
            const result = await db.query(`SELECT id, TO_CHAR(date,'YYYY-MM-DD') as date, flight, type, reg, "from", "to", TO_CHAR(std,'HH24:MI') as std, to_char(sta,'HH24:MI') as sta, seats, co_id, co_iata FROM flights WHERE co_id=$1 AND date=$2::date ORDER BY "from" ASC, std ASC`, [req.query.id, date]);
...
export async function insertCompanyItems(req, res, next) {
    let result;
    try {
        if (!req.params)
...
export async function updateCompanyItem(req, res, next) {
    if (!req.params)
        throw { status: 400, data: "Bad request. No params found." };
    try {
        const data = req.body.value;
        if (req.params.type === "flights") {
            await db.query(`UPDATE flights SET date=$2::date, flight=$3, type=$4, reg=$5, "from"=$6, "to"=$7, std=$8, sta=$9, seats=$10 WHERE id=$1`, [data.id, data.date, data.flight, data.type, data.reg, data.from, data.to, data.std, data.sta, data.seats]);
...
export async function deleteCompanyItem(req, res, next) {
...
export async function getItemImgUrl(req, res, next) {
    try {
        if (!fs.existsSync(`uploads/itm/${req.params.url}`))
...
export async function deleteItemImg(req, res, next) {
    if (!req.body)
